...
import { SOLIDITY_COURSE_ID, SOLIDITY_BASIC_LESSON_ID_14 } from '@site/src/constants/course';

# WTF Solidity: 14. Abstract and Interface

<QuizProgress courseId={SOLIDITY_COURSE_ID} lessonId={SOLIDITY_BASIC_LESSON_ID_14}></QuizProgress>
...
版并且减少代码冗余。我们还讲了`ERC721`接口合约`IERC721`，以及如何利用它与无聊猿`BAYC`合约进行交互。

<QuizForm courseId={SOLIDITY_COURSE_ID} lessonId={SOLIDITY_BASIC_LESSON_ID_14}></QuizForm>