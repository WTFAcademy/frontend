...
const Profile = (props: TProps) => {
  ...
  if (!isLogin) {
    return (
      <Link to="/login">
        <Button className={cn("h-9 mr-3", { hidden: props.mobile })}>
          <Translate id="profile.Login.button">登录</Translate>
        </Button>
      </Link>
    );
  }
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className={cn("relative h-8 w-8 rounded-full mr-3", {
            hidden: props.mobile,
          })}
        >
      ...
      <DropdownMenuContent>
        ...
        <DropdownMenuItem>
          ...
          <Link
            to="/personal"
            className="text-popover-foreground hover:text-popover-foreground hover:decoration-transparent"
          >
        ...
        <DropdownMenuItem onClick={handleSignOut}>
          ...
          <span>
            <Translate id="profile.Logout.button">登出</Translate>
          </span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
