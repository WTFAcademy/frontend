...
const QuizBridge=({index,quiz,control}:{quiz:IQuiz,index:number,control:any})=>{
    if(quiz.meta.type==='select'){
     return <QuizSelect control={control} name={`select${index}`} quiz={quiz}/>
    }
    if(quiz.meta.type==='multiple-select'){
        return <QuizMultipleSelect control={control} name={`multiple-select${index}`} quiz={quiz}/>
    }
...
const Preview = ({quizzes}: { quizzes: IQuiz[] }) => {
    ...
    return (
        <FormProvider methods={methods}>
                {quizzes.map((quiz,index)=><QuizBridge control={methods.control} index={index} quiz={quiz} key={index}/>)}
        </FormProvider>
    )
}
