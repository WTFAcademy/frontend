...
const AuthProvider = ({ children }: TProps) => {
  const [isGithubLogin, setIsGithubLogin] = useLocalStorageState(
    STORAGE_WTF_SIGN_IN_METHOD_GITHUB,
    {
      defaultValue: false,
    },
  );
  const [isWalletLogin, setIsWalletLogin] = useLocalStorageState(
    STORAGE_WTF_SIGN_IN_METHOD_WALLET,
    {
      defaultValue: false,
    },
  ...
  const handleSignWithWallet = (
    ...
    if (data?.token) {
      ...
      options.useLocationHref
        ? history.push(options.customPath || window.location.href)
        : history.push("/");
    }
  };
