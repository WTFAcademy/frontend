...
const Main = (props: TProps) => {
    ...
    const handleBinding = async () => {
        ...
        const res = await bindWallet({mesData: message, signData, wallet: address});
        if (res.code !== 0) {
            setBindError(true);
            return;
        }
    ...
    const rightButton = () => {
        if (unsupported) {
            return (<Button variant={unsupported && !disabled ? "destructive" : "ghost"}
                            onClick={openChainModal}>切换网络</Button>)
        }
                ...
                {(active || completed) && (
                    <div className="flex items-center">
                        {connected && !chain.unsupported && (
                            <div
                                className={clsx('cursor-pointer mr-2 text-[14px] lg:text-[16px]', {"text-black": connected && isBinding && !isErrorWallet})}
                                onClick={openAccountModal}>
                                {account.displayName}
                            </div>
                        )}
