    ...
    return (
        <>
            <StepCard>
                <div className="w-full flex justify-between">
                    ...
                    {!requestInfoLoading && active &&
                        <ArrowRightCircleIcon className="h-6 w-6 text-white cursor-pointer" onClick={handleLogin}/>}
                    {active && requestInfoLoading && <Spinner loading className="w-6 h-6"/>}
                    {completed && (
                        <div>{username}</div>
                    )}
                </div>
            ...
            {isLogin && !requestInfoLoading && !canGraduate &&
                <p className="ml-1 mt-2 text-[#D03838]">请先完成相关课程！</p>}
        </>
    )
}
